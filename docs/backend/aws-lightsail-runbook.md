...
1. `sudo bash infra/aws/setup-instance.sh` on the fresh instance (swap,
   Docker, unattended upgrades, SSH hardening).
...
- **Access:** SSH is key-only, restricted in the Lightsail firewall to
  operator IPs. AWS console access for the MSP goes through an IAM identity
  with least privilege — never share the root user.
